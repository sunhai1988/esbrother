 ...
 * 
 * <p>
 * The central method is <code>execute</code>, supporting Hibernate access code
 * implementing the {@link HibernateCallback} interface. It provides Hibernate
 * Session handling such that neither the HibernateCallback implementation nor
 * the calling code needs to explicitly care about retrieving/closing Hibernate
 * Sessions, or handling Session lifecycle exceptions. For typical single step
 * actions, there are various convenience methods (find, load, saveOrUpdate,
 * delete).
 * 
 * <p>
 ...
 * <code>SessionFactory.getCurrentSession()</code>). The major advantage is its
 * automatic conversion to DataAccessExceptions, the major disadvantage that no
 * checked application exceptions can get thrown from within data access code.
 * Corresponding checks and the actual throwing of such exceptions can often be
 * deferred to after callback execution, though.
 * 
 * <p>
 ...
 * PlatformTransactionManager is a configuration issue: For example, switching
 * to JTA is just a matter of Spring configuration (use JtaTransactionManager
 * instead) that does not affect application code.
 * 
 * <p>
 ...
 * 
 * <p>
 * Note that operations that return an Iterator (i.e. <code>iterate</code>) are
 * supposed to be used within Spring-driven or JTA-driven transactions (with
...
public class HibernateTemplate extends HibernateAccessor implements
	 ...
	 * created if none is found, which needs to be closed at the end of the
	 * operation. If <code>false</code>, an {@link IllegalStateException} will
	 * get thrown in this case.
	 * 
	 ...
	 * <code>close</code> calls and automatically applying query cache settings
	...
	public List findByNamedParam(final String queryString,
			final String[] paramNames, final Object[] values, final int pgnum,
			final int pgsize) throws DataAccessException {
		...
		return (List) execute(new HibernateCallback() {
			public Object doInHibernate(Session session)
				...
				Query queryObject = session.createQuery(queryString);
				if ((pgnum != -1) && (pgsize != -1)) {
					queryObject.setFirstResult((pgnum - 1) * pgsize);
					queryObject.setMaxResults(pgsize);
				}
				prepareQuery(queryObject);
				if (values != null) {
					for (int i = 0; i < values.length; i++) {
	...
	public List findByExample(final Object exampleEntity,
		...
		return (List) execute(new HibernateCallback() {
			public Object doInHibernate(Session session)
					throws HibernateException {
				Criteria executableCriteria = session.createCriteria(exampleEntity
						.getClass());
